...
from models.model_AMRN import Model
import torch
...
class Trainer(object):
    def __init__(self, params):
        ...
        self.model_path = os.path.join(self.params['cache_dir']+'_amrn')
    ...
    def train_one_epoch(self, i_epoch):
        ...
        loss_sum = 0
        t1 = time.time()
        for i_batch, (frame_vecs, frame_n, ques_n, labels, regs, idxs, windows, gt_windows) in enumerate(self.train_loader):
            ...
            # Forward pass
            batch_loss, predict_score, predict_reg = self.model(frame_vecs, frame_n, ques_n, labels, regs, idxs)
    ...
    def evaluate(self, data_loader):
        ...
        self.model.eval()
        for i_batch, (frame_vecs, frame_n, ques_n, labels, regs, idxs, windows, gt_windows) in enumerate(data_loader):
            frame_vecs = frame_vecs.to(self.device)
            frame_n = frame_n.to(self.device)
            ques_n = ques_n.to(self.device)
            labels = labels.to(self.device)
