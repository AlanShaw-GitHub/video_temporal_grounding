...
sys.path.append('..')
import json
from dataloaders.dataloader_ABLR import Loader
from models.model_ABLR import Model
import time
...
class Trainer(object):
    ...
    def train_one_epoch(self, i_epoch):
        ...
        for frame_vecs, frame_n, ques_vecs, ques_n, labels, gt_windows in self.train_loader.generate():
            ...
            # Forward pass
            _, batch_loss = self.sess.run([self.train_proc, self.model.loss], feed_dict=batch_data)
            i_batch += 1
            loss_sum += batch_loss
    ...
    def evaluate(self, data_loader):
        ...
        for frame_vecs, frame_n, ques_vecs, ques_n, labels, gt_windows in data_loader.generate():
            ...
            # Forward pass
            batch_loss, predicted_position = self.sess.run(
                                        [self.model.test_loss, self.model.predicted_position], feed_dict=batch_data)

            for i in range(batch_size):
                predict_score = np.array([1])
                predict_windows = np.array([predicted_position[i]])
                # print(predict_score)
